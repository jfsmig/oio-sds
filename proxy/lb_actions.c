...
static void
_on_id(struct oio_lb_selected_item_s *sel, gpointer u)
{
	GPtrArray *ids = u;
...
_lb(struct req_args_s *args, const char *srvtype)
{
	...
	GError *err = oio_lb_pool__poll(pool, NULL, _on_id, ids, &flawed);
	if (err) {
		g_prefix_error(&err,
...
_poll(struct req_args_s *args, struct json_object *body)
{
	...
	GPtrArray *ids = g_ptr_array_new_with_free_func(g_free);
	gboolean flawed = FALSE;
	err = oio_lb__patch_with_pool(lb, pool, avoid, known, _on_id, ids, &flawed);
	if (err) {
		g_prefix_error(&err,
				"found only %u services matching the criteria: ", ids->len);
		code = _reply_common_error(args, err);
